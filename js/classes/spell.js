...
class TwowEnrageAura extends Aura {
    constructor(player, id) {
        super(player, id);
        this.duration = 8;
    ...
    use() {
        ...
        /* start-log */ if (this.player.logging) this.player.log(`${this.name} applied. dmgmod ${this.player.stats.dmgmod}`); /* end-log */
    }
    step() {
        if (step >= this.timer) {
            this.uptime += (this.timer - this.starttimer);
...
class PotentVenoms extends Aura {
    ...
    step() {
        while (step >= this.nexttick) {
            ...
            /* start-log */ if (this.player.logging) this.player.log(`${this.name} (${this.stacks} stacks) tick for ${(dmg / 4).toFixed(2)}`); /* end-log */
...
class DeathWish extends Aura {
    ...
    use(a, prepull = 0) {
        if (this.timer) this.uptime += (step - this.starttimer);
        this.timer = step + this.duration * 1000 - prepull;
        this.player.rage -= (this.player.altdreadnaughttwoset ? 0 : 10);
        this.player.timer = 1500;
        this.starttimer = step - prepull;
        this.player.updateDmgMod();
        this.maxdelay = rng(this.player.reactionmin, this.player.reactionmax);
    ...
    canUse() {
        return !this.timer && !this.player.timer && this.player.rage >= (this.player.altdreadnaughttwoset ? 0 : 10) && step >= this.usestep;
    }
    step() {
        if (step >= this.timer) {
            this.uptime += (this.timer - this.starttimer);
...
class Tempest extends Aura {
    ...
    use() {
        ...
        this.starttimer = step;
        this.player.updateHaste();
        //if (this.player.logging) this.player.log(`${this.name} applied`);
    }
    step() {
        if (step >= this.timer) {
            this.uptime += (this.timer - this.starttimer);
            ...
            this.firstuse = false;
            this.player.updateHaste();
            //if (this.player.logging) this.player.log(`${this.name} removed`);
        }
    }
}
